...
func main() {
	...
	fmt.Printf("publick: %v\n", pubkey)
	fmt.Printf("r: %v\n  rLen: %d\n", r, len(r.Bytes()))
	fmt.Printf("s: %v\n  sLen: %d\n", s, len(s.Bytes()))
	//	一般签名者签名后，需要将签名的 公钥，签名产生的r s 在网络中发送，
	//  在发送时，我们一般会把这些进行转换为字节流，在网络上传输
...
发送端用私钥签名：
创建椭圆曲线(elliptic.P256()) ---> 生成私钥(ecdsa.GenerateKey) --->
生成公钥(privateKey.PublicKey) ---> 对待签名的数据取哈希(sha256.Sum256) --->
对数据进行签名(数据哈希+私钥)(ecdsa.Sign)---> 对签名生成的"r s"序列化(r.Bytes(), s.Bytes())

接收端用公钥验证：
对签名生成的"r s"反序列化(r1.SetBytes(),s1.SetBytes) ---> 对已签名数据取哈希(sha256.Sum256)
---> 进行验证(需要公钥、数据哈希、r、s)(ecdsa.Verify)
