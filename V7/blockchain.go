...
func (bc *BlockChain) AvarageBlock(txs []*Transaction) {
	//首先要对区块中所打包的交易依次进行验证,只要有一笔交易是无效的,那么区块打包就会失败
	for _, tx := range txs {
		if !bc.VerifyTransaction(tx) {
			fmt.Println("矿工发现无效交易,创建区块失败")
			return
		}
	}
	db := bc.db
...
//该函数实现了:找到交易所要用到的UTXO的集合
func (bc *BlockChain) FindNeedUTXOs(from []byte,
	...
	for {
		...
		OUTPUT:
			...
			for i, output := range transaction.TXOutputs {
				...
				if bytes.Equal(output.PubKeyHash, from) {
					if total < amount {
			...
			for _, input := range transaction.TXInputs {
				...
				if bytes.Equal(SetPubkeyHash(input.PubKey), from) {
