...
//这个函数提供将传入的地址解码为对应的160位公钥哈希的功能
//因为Output中要写的是公钥的160位哈希,而不是地址
func GetPubkeyHashFromAdderss(address string) []byte {
	payload := base58.Decode(address)
	PubkeyHash160 := payload[1 : len(payload)-4]
	return PubkeyHash160
}

//该函数用来判断用户输入的地址是否合法
func IsVaildAddress(address string) bool {
	//func base58.CheckDecode(input string) (result []byte, version byte, err error)
	//CheckDecode函数可以检查地址的合法性,如果不合法,err != nil
	_, _, res := base58.CheckDecode(address)
	if res != nil {
		return false
	} else {
		return true
